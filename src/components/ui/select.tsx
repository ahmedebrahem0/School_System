...
>(({ className, children, error, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      // Base — matches Input design
      "flex h-11 w-full items-center justify-between gap-2",
      "rounded-[8px] border bg-white px-3 py-2",
      "text-[14px] text-zinc-900",
      "placeholder:text-zinc-400",
      // Border
      error ? "border-red-400 ring-1 ring-red-100" : "border-zinc-300",
      // Focus
      "focus:outline-none focus:ring-2",
      error
        ? "focus:ring-red-200 focus:border-red-400"
        : "focus:ring-[#3B82F6]/20 focus:border-[#3B82F6]",
      // Transition
      "transition-all duration-150",
      // Disabled
      "disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-zinc-50",
      // Arrow icon
      "[&>span]:line-clamp-1 [&>svg]:shrink-0 [&>svg]:text-zinc-400 [&>svg]:size-4",
      className
    )}
...
// ─── Content ─────────────────────────────────────────────────────────────────
// dropdown: shadow-md, rounded-10, bg white, border zinc-200
